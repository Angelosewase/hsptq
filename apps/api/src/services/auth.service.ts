...
export const JWT_SECRET = process.env.JWT_SECRET || "default-secret-key-for-mvp";
...
export async function loginStaff(login: string, passwordPlain: string) {
  ...
  const token = await sign({ id: staff.id, role: staff.role, type: "staff", exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 }, JWT_SECRET);
...
export async function loginPatientApp(phone: string, passwordPlain: string) {
  ...
  const token = await sign({ id: patient.id, type: "patient", exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 30 }, JWT_SECRET);
