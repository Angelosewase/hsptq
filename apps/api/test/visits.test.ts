...
describe("Visit Routes", () => {
  it("POST /visits should check in", async () => {
    const mockVisit = {
      id: "v1", patientId: "p1", departmentId: "d1", queueNumber: 5, source: "app", status: "waiting", createdAt: new Date(), calledAt: null, servedAt: null
    };
    ...
    const res = await app.request("/api/visits", {
      ...
      body: JSON.stringify({ patientId: "p1", departmentId: "d1", source: "app" })
    });
