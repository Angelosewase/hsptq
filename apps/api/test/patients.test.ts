...
import { createPatient, findPatientByPhone } from "../src/services/patient.service";
...
describe("Patient Routes", () => {
  it("POST /patients should return existing patient", async () => {
    const mockPatient = { id: "1", name: "Jane", phone: "+1234567890", nationalId: null, createdAt: new Date() };
  ...
  it("POST /patients should create new patient", async () => {
    const mockPatient = { id: "2", name: "John", phone: "+0987654321", nationalId: null, createdAt: new Date() };
    ...
    const res = await app.request("/api/patients", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "John", phone: "+0987654321", password: "secure" })
    });
